...
car::Anova(t2, type =2) #significant for 3 way interaction
summary(t2)
...
results #not enough power = 833 

b1.day.2 <- glm(StationKeeping ~ treatcode + daytime + landtype, family = "binomial",
                data = subset(queentracks_combined, dataset == "day"))
summary(b1.day.2)
anova(b1.day.2, test = "Chisq")


b1.day.2a <- emmeans::emmeans (b1.day.2, 
                               pairwise ~ treatcode, 
                               data = subset(queentracks_combined, 
                                             dataset == "day"))
b1.day.2a
b1.day.2b <- emmeans::emmeans (b1.day.2, 
                               pairwise ~ landtype, 
                               data = subset(queentracks_combined, 
                                             dataset == "day"))
b1.day.2b
...
b2.day.2 <- glm(BroadARS ~ treatcode + daytime + landtype, family = "binomial",
                data = subset(queentracks_combined, dataset == "day"))
summary(b2.day.2)
anova(b2.day.2, test = "Chisq")

b2.day.2a <- emmeans::emmeans (b2.day.2, 
                               pairwise ~ treatcode, 
                               data = subset(queentracks_combined, 
                                             dataset == "day"))
b2.day.2a
b2.day.2b <- emmeans::emmeans (b2.day.2, 
                               pairwise ~ landtype, 
                               data = subset(queentracks_combined, 
                                             dataset == "day"))
b2.day.2b
...
results #not enough power = 832

b3.day.2 <- glm(Rest ~ treatcode + daytime + landtype, family = "binomial",
                data = subset(queentracks_combined, dataset == "day"))
summary(b3.day.2)
anova(b3.day.2, test = "Chisq")

b3.day.2a <- emmeans::emmeans (b3.day.2, 
                               pairwise ~ treatcode, 
                               data = subset(queentracks_combined, 
                                             dataset == "day"))
b3.day.2a
b3.day.2b <- emmeans::emmeans (b3.day.2, 
                               pairwise ~ landtype, 
                               data = subset(queentracks_combined, 
                                             dataset == "day"))
b3.day.2b
...
results #not enough power = 917

b1.week.2 <- glm(Explore ~ treatcode + daytime + landtype, family = "binomial",
                data = subset(queentracks_combined, dataset == "week"))
summary(b1.week.2)
anova(b1.week.2, test = "Chisq")

b1.week.2a <- emmeans::emmeans (b1.week.2, 
                               pairwise ~ treatcode, 
                               data = subset(queentracks_combined, 
                                             dataset == "week"))
b1.week.2a

b1.week.2b <- emmeans::emmeans (b1.week.2, 
                               pairwise ~ daytime, 
                               data = subset(queentracks_combined, 
                                             dataset == "week"))
b1.week.2b

b1.week.2c <- emmeans::emmeans (b1.week.2, 
                                pairwise ~ landtype, 
                                data = subset(queentracks_combined, 
                                              dataset == "week"))
b1.week.2c
...
#Rest
b2.week <- glm(Rest ~ treatcode * daytime * landtype, family="binomial", 
               data=subset(queentracks_combined, dataset == "week")) 
summary(b2.week)
results <- calculate_model_metrics(b2.week)
results #not enough power = 917

b2.week.2 <- glm(Rest ~ treatcode + daytime + landtype, family = "binomial",
                 data = subset(queentracks_combined, dataset == "week"))
summary(b2.week.2)
anova(b2.week.2, test = "Chisq")

b2.week.2a <- emmeans::emmeans (b2.week.2, 
                                pairwise ~ treatcode, 
                                data = subset(queentracks_combined, 
                                              dataset == "week"))
b2.week.2a

b2.week.2b <- emmeans::emmeans (b2.week.2, 
                                pairwise ~ daytime, 
                                data = subset(queentracks_combined, 
                                              dataset == "week"))
b2.week.2b

b2.week.2c <- emmeans::emmeans (b2.week.2, 
                                pairwise ~ landtype, 
                                data = subset(queentracks_combined, 
                                              dataset == "week"))
b2.week.2c
...
#day 
#station keeping and BroadARS and Rest treatcode and landcover not the interaction were sig
# only make a plot for the treatcode since that is what we are most interested in 
...
ggplot(behav.dayPlot2, aes(x = treatcode, y = response_value, fill = treatcode)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = response_value - error, ymax = response_value + error),
                width = 0, position = position_dodge(width = 0.8)) +
  facet_wrap(~response_var, scales = "free_y") +  # Create a facet for each response variable
  ...
  scale_fill_manual(values = c("#3F612D", "#FFC800", "#49BEAA"),
                    breaks = c("C", "I", "Y"), 
                    labels = c("control", "imidacloprid", "cyantraniliprole")) +
  scale_x_discrete(limits = c("C", "I", "Y"),
                   labels = c("control", "imidacloprid", "cyantraniliprole"),
                   name = "") +
  theme(
    axis.text.x = element_text(size = 14),
...
# week
# Explore, Rest everyhing sig (interactions not tested) so just make plot for treatcodes
...
ggplot(behav.weekPlot2, aes(x = treatcode, y = response_value, fill = treatcode)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = response_value - error, ymax = response_value + error),
                width = 0, position = position_dodge(width = 0.8)) +
  facet_wrap(~response_var, scales = "free_y") +  # Create a facet for each response variable
...
#the dataset needs x and y coordinates added

spatialcombine <- cbind(queentracks.hour, queentracks.week, queentracks.day)

##  separating the dataset to make KD rasters 
controlDay <- subset(queentracks_combined, treatcode == "C" & daytime == "day")
controlNight <- subset(queentracks_combined, treatcode == "C" & daytime == "night")
imidDay <- subset(queentracks_combined, treatcode == "I" & daytime == "day")
imidNight <- subset(queentracks_combined, treatcode == "I" & daytime == "night")
cyanDay <- subset(queentracks_combined, treatcode == "Y" & daytime == "day")
cyanNight <- subset(queentracks_combined, treatcode == "Y" & daytime == "night")
...
datasetList <- list(controlDay, controlNight, imidDay, imidNight, cyanDay, cyanNight)
names(datasetList) <- c("controlDay", "controlNight", "imidDay", "imidNight", 
                        "cyanDay", "cyanNight")
...
controlDay <- makeRaster(datasetList[[1]])
controlNight <- makeRaster(datasetList[2])
imidDay <- makeRaster(datasetList[[3]])
imidNight <- makeRaster(datasetList[4])
cyanDay <- makeRaster(datasetList[[5]])
cyanNight <- makeRaster(datasetList[6])
